...
#         weight = self.weight
#         out = F.linear(input, weight, bias=self.bias)
...
class FiLMSiren(nn.Module):
    ...
    def forward(self, input, style, weights_delta=None):
        batch, features = style.shape
        # print(weights_delta is None)
        weight = self.weight if weights_delta is None else self.weight * (1 + weights_delta)
...
class SirenGenerator(nn.Module):
    def __init__(self, D=8, W=256, style_dim=256, input_ch=3, input_ch_views=3, output_ch=4,
        ...
        # # input_ch_views: number of camera pose parameters,  camera azimuth and evaluation angles (phi, theta)
    ...
    def forward(self, x, styles, weights_delta=None):
        input_pts, input_views = torch.split(x, [self.input_ch, self.input_ch_views], dim=-1)
        mlp_out = input_pts.contiguous()

        for i in range(len(self.pts_linears)):      # len(self.pts_linears)=8
            if weights_delta:
                shape = self.pts_linears[i].weight.shape
                delta = weights_delta[i].view(shape)
            else:
                delta = None
            mlp_out = self.pts_linears[i](mlp_out, styles, delta)

        sdf = self.sigma_linear(mlp_out)
        mlp_out = torch.cat([mlp_out, input_views], -1)
        if weights_delta:
            shape = self.views_linears.weight.shape
            # print(shape)
            delta = weights_delta[-1].view(shape)
        else:
            delta = None
        out_features = self.views_linears(mlp_out, styles, delta)
...
class VolumeFeatureRenderer(nn.Module):
    def __init__(self, opt, style_dim=256, out_im_res=64, mode='train'):
        ...
        self.perturb = opt.perturb      # 1.
        self.offset_sampling = not opt.no_offset_sampling # Stratified sampling used otherwise
        self.N_samples = opt.N_samples   # 24 (in 64 times 64)
        self.raw_noise_std = opt.raw_noise_std  # 0.
        self.return_xyz = opt.return_xyz    # true
        self.return_sdf = opt.return_sdf    # true
        self.static_viewdirs = opt.static_viewdirs  # true
        self.z_normalize = not opt.no_z_normalize   # true
        self.out_im_res = out_im_res    # image resolution : 64,,  change up to 128 to improve fidelity of 3d geo.
        self.force_background = opt.force_background
        self.with_sdf = not opt.no_sdf
        # sdf (False) or nerf density (True), if false, sdf is further modulated by K to volume density.
    ...
    def run_network(self, inputs, viewdirs, styles=None, weights_delta=None):
        ...
        outputs = self.network(net_inputs, styles=styles, weights_delta=weights_delta)

        return outputs

    def render_rays(self, ray_batch, styles=None, return_eikonal=False, weights_delta=None):
        ...
        raw = self.run_network(normalized_pts, viewdirs, styles=styles, weights_delta=weights_delta)
    ...
    def render(self, focal, c2w, near, far, styles, c2w_staticcam=None, return_eikonal=False, weights_delta=None):
        ...
        rays = rays.float()
        rgb, features, sdf, mask, xyz, eikonal_term = self.render_rays(rays, styles=styles,
    ...
    def forward(self, cam_poses, focal, near, far, styles=None, return_eikonal=False, weights_delta=None):
        rgb, features, sdf, mask, xyz, eikonal_term = self.render(focal, c2w=cam_poses, near=near, far=far,
                                                                  styles=styles, return_eikonal=return_eikonal,
                                                                  weights_delta=weights_delta)
